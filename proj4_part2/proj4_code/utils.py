...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
    ...
    im_left = torch.randint(low=0, high=2, size=(H, W))
    im_right = im_left.clone().detach()
    im_left = im_left.type(torch.FloatTensor)
    im_right = im_right.type(torch.FloatTensor)

    im_right[(H//4)+1:(3*H)//4, ((W//4)+1-disparity):(((3*W)//4)-disparity)] = im_right[(H //
                                                                                         4)+1:(3*H)//4, (W//4)+1:((3*W)//4)]
    im_right[((3*H)//4)-disparity:((3*H)//4), W//4 +
             1:(3*W//4)] = torch.randint(low=0, high=2, size=(disparity, (3*W)//4 - 1 - W//4))  # .random_(0, 2)
    im_right = torch.stack(3*[im_right], dim=2)
    im_left = torch.stack(3*[im_left], dim=2)
    ############################################################################
    # Student code end
    ############################################################################
