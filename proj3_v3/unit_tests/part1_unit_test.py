...
def test_objective_func():
    ...
    assert np.allclose(np.sum(output), np.sum(
        test_output1), atol=1e-8) or np.allclose(np.sum(output), np.sum(test_output2), atol=1e-8)
...
def test_estimate_camera_matrix():
    ...
    initial_guess_P = np.matmul(initial_guess_K, np.matmul(
        initial_guess_R, initial_guess_I_t))
    pts2d_path = '../data/pts2d-pic_b.txt'
    pts3d_path = '../data/pts3d.txt'
