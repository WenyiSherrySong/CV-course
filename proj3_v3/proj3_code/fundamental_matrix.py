...
def point_line_distance(line, point):
    ...
    ##############################
    error = (a*u + b*v + c*w)/np.sqrt(a**2 + b**2)
    ##############################
...
def signed_point_line_errors(x_0s, F, x_1s):
    ...
    x_0s = np.array(x_0s)
    x_1s = np.array(x_1s)
    if x_0s.shape[1] != 3:  # converting non homogenous to homogeneous
        x_0s = np.insert(x_0s, 2, values=1, axis=1)
        x_1s = np.insert(x_1s, 2, values=1, axis=1)
    F = np.array(F)
    for i in range(0, x_0s.shape[0]):
        temp = np.dot(F, x_1s[i, :])
        errors.append(point_line_distance(temp, x_0s[i, :]))
        temp = np.dot(F.transpose(), x_0s[i, :])
        errors.append(point_line_distance(temp, x_1s[i, :]))
