...
import proj3_code.feature_matching as feature_matching
...
def calculate_num_ransac_iterations(prob_success, sample_size, ind_prob_correct):
    ...
    """
    ##############################
    num_samples = math.log(1-prob_success)/math.log(1 -
                                                    (ind_prob_correct)**sample_size)
    ##############################
...
def find_inliers(x_0s, F, x_1s, threshold):
    ...
    # errors1 = errors[0:errors.shape[0]:2]  # even elements
    # errors2 = errors[1:errors.shape[0]:2]  # odd elements
    # final_error = np.maximum(errors1, errors2)
    # inliers = np.argwhere((threshold) >= final_error)
    errors = fundamental_matrix.signed_point_line_errors(x_0s, F, x_1s)
    errors = np.abs(np.array(errors))
    errors1 = errors[0:errors.shape[0]:2]  # even elements
    errors2 = errors[1:errors.shape[0]:2]  # odd elements
    final_error = (errors1 + errors2)/2
    inliers = np.argwhere((threshold) >= final_error)
...
def ransac_fundamental_matrix(x_0s, x_1s):
    ...
    itrtions = calculate_num_ransac_iterations(.99, 9, .9)
    inliers_og = np.array([1])
    best_F = np.random.rand(3, 3)
    # print(x_0s.shape)
    # if not(x_0s.shape[1] == 3):  # converting non homogenous to homogeneous
    #     x_0s = np.insert(x_0s, 2, values=1, axis=1)
    #     x_1s = np.insert(x_1s, 2, values=1, axis=1)

    for i in range(0, itrtions):
        random_indices = np.random.choice(x_0s.shape[0], 9, replace=False)
        x_0s_input = np.array(x_0s[random_indices])
        x_1s_input = np.array(x_1s[random_indices])
        F_initial = solve_F(x_0s_input, x_1s_input)
        inliers_n = find_inliers(x_0s, F_initial, x_1s, 1)
        if len(inliers_n) > len(inliers_og):
            inliers_og = inliers_n
            best_F = F_initial

    inliers_indices = find_inliers(x_0s, best_F, x_1s, 1)
