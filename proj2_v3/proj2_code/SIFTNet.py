...
class HistogramLayer(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        ** You will receive extra-credit if you successfully implement this
        ...
        cosine_max = cosines.max(1, keepdim = True).values
        binary_occupancy = (cosines == cosine_max).float()
        per_px_histogram = torch.mul(binary_occupancy,im_grads.norm(dim=1))
...
def angles_to_vectors_2d_pytorch(angles: torch.Tensor) -> torch.Tensor:
    ...
    angle_vectors = torch.stack((angles_cos,angles_sin),dim=1)
...
class SIFTOrientationLayer(nn.Module):
    ...
    def get_orientation_bin_weights(self) -> torch.nn.Parameter():
        ...
        pi = np.pi
        angles = torch.Tensor([pi/8, 3*pi/8, 5*pi/8, 7*pi/8, 9*pi/8, 11*pi/8, 13*pi/8, 15*pi/8, 0, pi/2]) 
        # 0, pi/2 are for Ix and Iy                           
        weight_param = nn.Parameter(angles_to_vectors_2d_pytorch(angles).unsqueeze(2).unsqueeze(3))
...
def get_sift_subgrid_coords(x_center: int, y_center: int):
    ...
    x = x_center
    y = y_center
    x_grid = np.array([x-6,x-2,x+2,x+6,x-6,x-2,x+2,x+6,x-6,x-2,x+2,x+6,x-6,x-2,x+2,x+6])
    y_grid = np.array([y+6,y+6,y+6,y+6,y+2,y+2,y+2,y+2,y-2,y-2,y-2,y-2,y-6,y-6,y-6,y-6])
    # x_grid = x_grid.reshape(16,)
    # y_grid = y_grid.reshape(16,)
    # print(x_grid.size())
...
def get_siftnet_features(img_bw: torch.Tensor, x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    for i in range(0,x.shape[0]):
        xcoords, ycoords = get_sift_subgrid_coords(x[i],y[i])
        opt1 = output[0, :, ycoords, xcoords].t().detach().numpy().flatten()
        opt1 = (opt1 / np.linalg.norm(opt1))**.9
        features.append(opt1)
        fvs = np.stack(features)
    # print("llx")
