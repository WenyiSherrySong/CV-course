...
class HarrisNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.net = nn.Sequential(image_gradients_layer,ChannelProductLayer(),SecondMomentMatrixLayer(),CornerResponseLayer(),NMSLayer()) # <--replace this with your implementation
...
class ChannelProductLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print("CPL")
        # print(x.size())
        I_xx1 = torch.mul(x[:,0,:,:],x[:,0,:,:])
        I_yy1 = torch.mul(x[:,1,:,:],x[:,1,:,:])
        I_xy1 = torch.mul(x[:,0,:,:],x[:,1,:,:])
        output = torch.stack([I_xx1, I_yy1, I_xy1], dim = 1)
        # print(output.size())
...
class SecondMomentMatrixLayer(torch.nn.Module):
    ...
    def __init__(self, ksize: torch.Tensor = 7, sigma: torch.Tensor = 5):
        ...
        self.gaussiankernel = get_gaussian_kernel(ksize,sigma)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print("SMML")
        # print(x.size())
        GK = self.gaussiankernel # k x k dimesnions
        GK_np = GK.numpy() # i find it easier to play with dim in np
        B = np.reshape(GK_np, (1, 1, self.ksize, self.ksize))
        B = np.tile(B, (3, 1, 1, 1))
        GK = torch.from_numpy(B)
        conv_gk = nn.Conv2d(3,3,kernel_size =self.ksize,padding=self.ksize//2,groups=3, bias=False, padding_mode='zeros')
        GK = GK.type(torch.FloatTensor)
        GK = torch.nn.Parameter(GK) 
        conv_gk.weight = GK
        output = conv_gk(x)
        # print(output.size())
...
class CornerResponseLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        alpha = self.alpha
        determinant = torch.mul(S_xx,S_yy) - torch.mul(S_xy,S_xy)
        trace = S_xx + S_yy
        output = determinant - alpha*(trace**2)
...
class NMSLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_pos = torch.where(x > median,x,dummy)
        pool_k = nn.MaxPool2d( 7, stride=1, padding = 7//2)
        x_pooled = pool_k(x_pos)
        dummy1 = torch.ones_like(x_pooled)
        x_binarized = torch.where(x_pooled == x, dummy1 , dummy)
        output = torch.mul(x,x_binarized)
        # print(output.size())
...
def get_interest_points(image: torch.Tensor, num_points: int = 3000) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    R_index = R_index[:num_points] # Please change here to update desired # of interest points
...
def remove_border_vals(img, x: torch.Tensor, y: torch.Tensor, c: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    mask = (x > 8) & (x < (img.shape[3] - 8)) & (y > 8) & (y < (img.shape[2] - 8))
    x = x[mask]
    y = y[mask]
    c = c[mask]
    # print(x.size())
