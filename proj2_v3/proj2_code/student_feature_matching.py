...
def compute_feature_distances(features1, features2):
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    dists = np.random.randn(features1.shape[0],features2.shape[0])
    for i in range(0,features1.shape[0]):
        for j in range(0,features2.shape[0]):
...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    values = np.sort(dists, axis = 1)
    confidences = values[:, 0] / values[:, 1] # taking the ratio
    idx = confidences <= .78   
    confidences = confidences[idx]
    indices = np.argsort(dists, axis = 1)   
    matches = np.stack([np.argwhere(idx).flatten(), indices[idx, 0]], axis = 1)
