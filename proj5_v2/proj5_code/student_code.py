...
def pairwise_distances(X, Y):
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    train_square = np.sum(X**2, axis=1).reshape(N, 1)
    test_square = np.sum(Y**2, axis=1).reshape(1, M)
    D = np.sqrt(np.abs(train_square + test_square - (2*np.dot(X, Y.T))))
...
def nearest_neighbor_classify(train_image_feats, train_labels,
    ...
    # TODO: YOUR CODE HERE                                                      #
    #############################################################################
    num_train = train_image_feats.shape[0]
    num_test = test_image_feats.shape[0]
    dists = pairwise_distances(train_image_feats, test_image_feats)
    for i in range(0, num_test):
        z = np.argsort(dists[:, i])[:k]  # k nearest neighbors
        # print(z)
        (values, counts) = np.unique(z, return_counts=True)
        ind = np.argmax(counts)  # finding most common element
        # print(train_labels[values[ind]])
        test_labels.append(train_labels[values[ind]])
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def kmeans(feature_vectors, k, max_iter=100):
    ...
    _, unique_indices = np.unique(feature_vectors, axis=0, return_index=True)
    rng = np.random.choice(unique_indices, k, replace=False)
    centroids = feature_vectors[rng, :]

    for i in range(max_iter):
        D = pairwise_distances(centroids, feature_vectors)
        centroid_indexes = np.argmin(D, axis=0)
        for c in range(k):
            centroids[c] = np.mean(feature_vectors[centroid_indexes == c], 0)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return centroids
...
def build_vocabulary(image_arrays, vocab_size, stride=20):
    ...
    total_SIFT_features = np.zeros((8, 128))  # 200,128
    index = 0
#     print((np.array(image_arrays)).shape)

#     print(x, y)

    for i in range(len(image_arrays)):
        # print(np.array(image_arrays, dtype='float32').shape)
        image_tensor = torch.from_numpy(
            np.array(image_arrays[i], dtype='float32'))
        # print(i)
        # print(image_tensor.shape)
        x = np.arange(10, image_tensor.shape[0] - 10, stride)
        y = np.arange(10, image_tensor.shape[1] - 10, stride)
        ...
        image_tensor = image_tensor.reshape(
            (1, 1, image_tensor.shape[0], image_tensor.shape[1]))
        # print(image_tensor.shape)
        fvs = get_siftnet_features(image_tensor, y_1, x_1)
        if i == 1:
            total_SIFT_features = fvs
        else:
            total_SIFT_features = np.concatenate(
                (total_SIFT_features, fvs), axis=0)
    vocab = kmeans(total_SIFT_features.astype('float32'), vocab_size)
...
def kmeans_quantize(raw_data_pts, centroids):
    ...
    # rng = np.random.choice(unique_indices, k, replace=False)
    # centroids = feature_vectors[rng, :]
    # for i in range(0,raw_data_pts)
    indices = np.zeros(raw_data_pts.shape[0])
...
def get_bags_of_sifts(image_arrays, vocabulary, step_size=10):
    ...
    for i in range(0, len(image_arrays)):
        ...
        tensor = torch.from_numpy(image).reshape(
            (1, 1, image.shape[0], image.shape[1]))
        x = np.arange(10, image.shape[0] - 10, step_size)
        y = np.arange(10, image.shape[1] - 10, step_size)
        x_1, y_1 = np.meshgrid(x, y)
        x_1 = x_1.flatten()
        y_1 = y_1.flatten()
        features = get_siftnet_features(tensor, y_1, x_1)
        indices = kmeans_quantize(features, vocab)
        unique, counts = np.unique(indices, return_counts=True)
        # unique = unique / np.linalg.norm(unique)
        for u, c in zip(unique, counts):
            feats[i, u] = c
#     feats = feats/np.linalg(feats, axis=1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return feats
