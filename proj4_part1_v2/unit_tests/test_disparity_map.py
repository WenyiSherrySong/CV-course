...
def test_disparity_deltafn_success():
    ...
    # check the size
    # print(torch.eq(im_left, im_right))
    # print(im_left)
    # print(im_right)
...
def test_disparity_deltafn_failure():
    ...
    # we should get two non-zero values in the disparity map
    nonzero_disp = torch.nonzero(disp_map).data
    # print("MSB:", disparity_expected-1)
    print(nonzero_disp)
...
def test_calculate_cost_volume():
    ...
    cost_volume = calculate_cost_volume(
        left_image, right_image, 4, sad_similarity_measure, 1)
    ...
    cost_volume = calculate_cost_volume(
        left_image, right_image, 7, sad_similarity_measure, 1)

    assert np.all(np.isclose(
        cost_volume[8, 7, :].cpu().numpy(), [3, 3, 3, 3, 3, 3, 0]))
    ...
    cost_volume = calculate_cost_volume(
        left_image, right_image, 7, sad_similarity_measure, 2)
